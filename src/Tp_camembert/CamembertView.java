...
public class CamembertView extends JComponent implements MouseListener,
	...
	public CamembertView(CamembertAdapter m) {
		...
		//ABAPOE refaire l'heritage
		//View gets its methods from Icamembert when using Model
	...
	public void mousePressed(MouseEvent arg0) {
		...
		for(int i = 0; i < this.arcs.size(); i++) {
			if(arcs.get(i).contains(prevPosX, prevPosY)) {
				selectPie(i);
			}
		}
...
public static void main(String[] a) {
        ...
        // Model model;
        CamembertAdapter m = new CamembertAdapter();
		...
		m.addItem("Loyer", "un chambre Chez le Crous", 400);
		m.addItem("Nourriture", "Courses chez carrefour", 250);
		m.addItem("Sport", "Matériel de Ski", 200);
		m.addItem("Sorties", "Soirée étudiantes", 150);
		m.addItem("Tourisme", "Un voyage en espagne", 220);
		m.setUnit("Euros");
		m.setTitle("Budget mensuel");
		
		TableModel tableModel = m.getTableModel();
		...
		CamembertView view = new CamembertView(m);
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
		    ...
		    public void valueChanged(ListSelectionEvent lse) {
		        if (!lse.getValueIsAdjusting()) {
		            controller.setSelectedPie(table.getSelectedRow());
	...
	//	window.add(scrollpane);
		...
		window.setTitle("Budget Camembert");
